...
class SchurPCuT(AuxiliaryOperatorPC):
    def form(self, pc, V, U):
        ...
        A = (
              ...
              + inner(dot(u_n, grad(T)), s) * dx
              + inner(dot(u, grad(T_n)), s) * dx
                    )
        ...
        if dim == 2 and testproblem == "ldc":
            bcs = [DirichletBC(V.function_space().sub(0), 0, "on_boundary"),
                   DirichletBC(V.function_space().sub(1), 0, (1,2)),
               ]
    ...
    def form(self, pc, V, U):
        ...
        if discr in ["rt", "bdm"]:
            ...
            if bcs_ids_dont_apply is not None:
                A_DG += (
                    - inner(outer(u, n), 2/Re*sym(grad(v))) * ds(bcs_ids_dont_apply)
                    + 1/Re*(sigma/h)*inner(u, v) * ds(bcs_ids_dont_apply)
                   )
...
class SchurPCT(AuxiliaryOperatorPC):
    def form(self, pc, V, U):
        T = U
        s = V
        state = self.get_appctx(pc)['state']
...
if checkpoint:
    try:
        if len(args.S) == 1 or len(args.Pm) == 1:
...
# Define MMS for smooth problem
if testproblem == "smooth":
    u_ex = as_vector([cos(y), sin(x)]) 
    B_ex = as_vector([sin(y), cos(x)])
    T_ex = cos(x)
    p_ex = sin(x)
...
if solution_known:
    ...
    #z.split()[2].assign(T_ex_)
    #z.split()[3].assign(B_ex_)
...
if discr in ["rt", "bdm"]:
    h = CellVolume(mesh)/FacetArea(mesh)
    sigma = Constant(10) * Z.sub(0).ufl_element().degree()**2  # penalty term
...
if linearisation == "newton":
    ...
    J_picard = (
          J_newton
        - S * inner(vcross(w_B, E), v) * dx  # J_tilde
        - S * inner(vcross(w_B, scross(u, B)), v) * dx  # D_1_tilde
        - S * inner(vcross(B, scross(u, w_B)), v) * dx  # D_2_tilde
